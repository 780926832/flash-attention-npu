        ...
        )
{
    ...
    auto hostElapsedUs = std::chrono::duration_cast<std::chrono::microseconds>(
        hostEnd - hostStart).count();
    printf("[flash_api_host_pre_kernel] elapsed=%.3f ms\n", hostElapsedUs / 1000.0);
    ...
    static_cast<int>(is_varlen_q),
    static_cast<int>(is_varlen_kv),
    static_cast<int>(flashDecodeFlag),
    launchBlockDim,
    blockDim,
    batch_size,
    seqlen_q,
